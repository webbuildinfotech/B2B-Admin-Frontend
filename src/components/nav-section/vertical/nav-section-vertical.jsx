...
export function NavSectionVertical({
  ...
  // Guard clause to avoid rendering if data is empty
    ...
    <Stack component="nav" className={navSectionClasses.vertical.root} sx={{ ...cssVars, ...sx }}>
      <NavUl sx={{ flex: '1 1 auto', gap: 'var(--nav-item-gap)' }}>
        {data.map((group, index) => {
          // Ensure items exist before attempting to access them
          const items = group.items || [];
          const key = group.subheader || items[0]?.title || `group-${index}`;
